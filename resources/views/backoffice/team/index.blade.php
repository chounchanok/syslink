    ...
    <body class="py-5">
        ...
        <div class="flex">
            ...
            <div class="content">
                ...
                <div class="grid grid-cols-12 gap-6 mt-5">
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                        <a class="btn shadow-md mr-2 {{ auth()->guard('admin')->user()->role == 'superadmin' ? 'btn-primary' : 'btn-secondary disabled-link' }}"
                                    href="{{ auth()->guard('admin')->user()->role == 'superadmin' ? url('/team/add') : '#' }}"
                            title="{{ auth()->guard('admin')->user()->permission == 1 ? '' : 'User does not have permission to edit data in the system' }}">Add New Team</a>
                                ...
                                <tr class="intro-x">
                                    ...
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            <a class="flex items-center mr-3" href="{{url("/team/list/$tm->id")}}"><i data-lucide="file-text" class="w-4 h-4 mr-1"></i>รายชื่อคนในทีม</a>
                                            @php
                                                $admin_now=Auth::guard('admin')->user();
                                                $check_role=$admin_now->role;
                                            @endphp
                                            @if ($check_role == 'superadmin'||auth()->guard('admin')->user()->permission == 1)
                                            <a class="flex items-center mr-3" href="{{url("/team/edit/$tm->id")}}"><i data-lucide="check-square" class="w-4 h-4 mr-1"></i>Edit</a>
                                            @endif
                                            @if ($check_role == 'superadmin')
                                            <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-team{{$tm->id}}" class="flex items-center mr-3" style="color: red"> <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> Delete</a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                ...
                <!-- BEGIN: Delete Confirmation Modal --><div class="text-center" style="display: none"> <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#button-modal-preview" class="btn btn-primary" id="auto-click-button">Show Modal</a> </div> <!-- END: Modal Toggle -->
                    <!-- BEGIN: Modal Content -->
                ...
                <div id="delete-team{{$item->id}}" class="modal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form action="{{url("/team/delete")}}" method="post">
                                @csrf
                            <div class="modal-body p-0">
