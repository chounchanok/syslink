    ...
    <body class="py-5">
        ...
        <div class="flex">
            ...
            <div class="content">
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                        ...
                        <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                            {{-- <div class="w-56 relative text-slate-500"> --}}
                                <input type="text" class="form-control w-56 box pr-10" name="search" id="search" value="{{@$search}}" onkeyup="myFunction()" placeholder="Search...">
                                ...
                                <a class="btn  shadow-md mr-2 {{ auth()->guard('admin')->user()->role == 'superadmin' ? 'btn-primary' : 'btn-secondary disabled-link' }}"
                                    href="{{ auth()->guard('admin')->user()->role == 'superadmin' ? url('/technician/add') : '#' }}" >Add New Technician</a>
                                ...
                                <tr class="intro-x">
                                    ...
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            ...
                                            <a class="flex items-center mr-3" href="{{url("/technician/edit/$ad->id")}}"><i data-lucide="check-square" class="w-4 h-4 mr-1"></i>Edit</a>
                                            @endif
                                            @if ($check_role == 'superadmin')
                                            <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-technician{{$ad->id}}" class="flex items-center mr-3" style="color: red"> <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> Delete</a>
                                            @endif
                                        </div>
                                    </td>
                                    {{-- <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            @php
                                                $admin_now=Auth::guard('admin')->user();
                                                $check_role=$admin_now->permission;
                                            @endphp
                                            @if ($check_role != 1)

                                            @else
                                            <a class="flex items-center mr-3" href="{{url("/technician/edit/$ad->id")}}"><i data-lucide="check-square" class="w-4 h-4 mr-1"></i>Edit</a>
                                            @if ($admin_now->role != 'admin')
                                            <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-technician{{$ad->id}}" class="flex items-center mr-3" style="color: red"> <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> Delete</a>
                ...
                <div id="delete-technician{{$item->id}}" class="modal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form action="{{url("/technician/delete")}}" method="post">
                                @csrf
                            <div class="modal-body p-0">
        ...
        <script>
            function searchDoc(search,project,status,sale,engineer,team){
                window.location.replace('/technician?search='+document.getElementById(search).value);
            }
        </script>
    </body>
</html>
