            ...
            <div class="content">
                ...
                <div class="grid grid-cols-12 gap-6 mt-5">
                    ...
                    <div class="intro-y col-span-12 lg:col-span-8">
                        <!-- BEGIN: Form Layout -->
                        <form action="{{url('/technician/edit/sub')}}" id="saveForm" method="POST" enctype="multipart/form-data">
                            @csrf
                        <div class="intro-y box p-5">
                            <input type="hidden" name="id" value="{{$engineer->id}}" id="">


                            <div>
                                <label for="crud-form-1" class="form-label">Username</label>
                                <input id="crud-form-1" type="text" class="form-control w-full" name="username" placeholder="Input text" value="{{$engineer->username}}">
                            </div>
                            <div>
                                ...
                                <input id="password" type="password" class="form-control w-full" {{ auth()->guard('admin')->user()->role == 'superadmin'?'':'disabled' }} name="password" placeholder="Input text" value="">
                            </div>
                            <div>
                                <label for="password_confirmation" class="form-label">Confirm Password</label>
                                <input id="password_confirmation" type="password" class="form-control w-full" {{ auth()->guard('admin')->user()->role == 'superadmin'?'':'disabled' }} required name="password_confirmation" placeholder="confirm password">
                            </div>
                            <div>
                                <label for="crud-form-1" class="form-label">Name</label>
                                <input id="crud-form-1" type="text" class="form-control w-full" required {{ auth()->guard('admin')->user()->role == 'superadmin' ? '':'disabled' }} name="name" value="{{$engineer->name}}" placeholder="name">
                            </div>
                            @php
                            ...
                            <div>
                                ...
                                <select name="team_id" id="" class="form-control">
                                    ...
                                    @foreach ($team as $tm)
                                    <option value="{{$tm->id}}" @if ($engineer->team_id == $tm->id) selected @endif>{{$tm->team_name}}</option>
                                    @endforeach
                                </select>
                            </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    saveButton.addEventListener('click', function(event) {
        ...
        if (password.value !== confirmPassword.value) {
            confirmPassword.setCustomValidity("Passwords do not match");
            confirmPassword.reportValidity(); // แสดงข้อผิดพลาดหากรหัสผ่านไม่ตรงกัน
        } else {
            confirmPassword.setCustomValidity(""); // ล้างข้อผิดพลาดก่อนยืนยัน
            ...
            Swal.fire({
                ...
                confirmButtonColor: '#3085d6', // Set the color for the confirm button
                cancelButtonColor: '#d33', // Set the color for the cancel button
                confirmButtonText: 'บันทึก',
                cancelButtonText: 'ยกเลิก'
            }).then((result) => {
                if (result.isConfirmed) {
                    document.getElementById('saveForm').submit(); // ส่งฟอร์มหากผู้ใช้ตอบ "บันทึก"
                }
            });
        }
    });
